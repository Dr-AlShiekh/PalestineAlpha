...
async function analyze() {
  ...
  try {
    ...
    if (!data.content || !Array.isArray(data.content)) throw new Error('Unexpected API response. Try again.');
...
Use real live data from web search. Return ONLY the JSON.`;
  ...
  try {
    ...
    const raw = data.content.filter(b=>b.type==='text').map(b=>b.text).join('').trim();
    const m = raw.match(/\{[\s\S]*\}/);
    if (!m) throw new Error('Could not parse brief. Try again.');
...
{"ticker":"GME","name":"GameStop","sector":"Retail","short_pct":"22.4%","float":"430M","dtc":"2.1","vol_ratio":"3.2x","score":78,"signal":"HOT SQUEEZE"}`;
  ...
  try {
    ...
    const data = await resp.json();
    if (data.error) throw new Error(data.error.message || 'API error');
    if (!data.content || !Array.isArray(data.content)) throw new Error(`API ${resp.status} — check your key has credits.`);
    const raw = data.content.filter(b=>b.type==='text').map(b=>b.text).join('').trim();
    let stocks;
    try { stocks = JSON.parse(raw); }
    catch {
      const m = raw.match(/\[[\s\S]*?\]/);
      if (!m) throw new Error('No results returned. Try again.');
      stocks = JSON.parse(m[0]);
    }
    if (!Array.isArray(stocks) || !stocks.length) throw new Error('Empty results — try again.');
